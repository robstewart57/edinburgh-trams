...
ggplot(data=dat, aes(x=year, y=growth, group=placeName, linetype = placeName, colour = placeName)) +
    geom_line(size=1.3) +
    theme_bw() +
    theme(aspect.ratio=3/3, axis.title=element_text(size=18,face="bold"), axis.text.y = element_text(size=18), axis.text.x = element_text(size=18), legend.title = element_blank()) +
